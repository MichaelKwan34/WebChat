...
export default function RegisterForm({ setView, setUsernameLogin, setPasswordLogin }) {
  ...
  async function isUsernameAvailable(username) {
    const res = await fetch(`http://localhost:3000/api/users/check-username?username=${username}`);
  ...
  async function isEmailAvailable(email) {
    const res = await fetch(`http://localhost:3000/api/users/check-email?email=${email}`);
    const data = await res.json();
    return data.available;
  }
