...
export default function SidebarContent({ socket, currentUser, activeTab, activeFriend, setActiveFriend, activeChat, setActiveChat, friends, setFriends, chats, setChats, currentSearch, setConversationId, setMessages }) {
  ...
  const handleFriendClick = async (friend) => {
    try {
      const resConversationId = await fetch(`${import.meta.env.VITE_API_URL}/api/conversations/${currentUser}/${friend}`);
      const dataConversationId = await resConversationId.json();
      setConversationId(dataConversationId.conversationId);

      const resMsg = await fetch(`${import.meta.env.VITE_API_URL}/api/messages/${dataConversationId.conversationId}`)
  ...
  const handleSearch = async (username) => {
    ...
    const res = await fetch (`${import.meta.env.VITE_API_URL}/api/users/find-user/${currentUser}/${username}`);
  ...
  const handleAddContact = async () => {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/add-contact/${currentUser}`, {
      method: "POST",
  ...
  async function fetchFriends() {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/${currentUser}/friends`);
  ...
  async function fetchChats() {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/users/${currentUser}/chats`);
